...
# INPUT
print('Reading inputs and functions...')
# files
...
# Taken from SO_tourist answer at: https://stackoverflow.com/questions/32485907/matplotlib-and-numpy-create-a-calendar-heatmap
def create_year_calendar(day_nums, day_vals, filename, color_dict=None, full_paths=None, part_paths=None, leg_title='track'):
...
def create_month_calendars(day_nums, day_vals, filename_base):
    for i in range(0, 12):
        ...
        # Save to file
        outname = filename_base.replace('.png', f'{i+1:02}.png')
...
def ann_calendar_text(acqs, full_paths, part_paths, yr_starts, pass_int):
...
def ann_calendar_graph(df_all, yr_dates, filename, color_dict, full_paths, part_paths):
    ...
    create_year_calendar(day_nums, day_vals, filename, color_dict=color_dict, full_paths=full_paths, part_paths=part_paths)

def month_calendar_graph(df_all, yr_dates, filename_base, color_dict):
    date_gen = pd.DataFrame({
        'date': yr_dates
        , 'colorid':0
    })
    ...
    df_dates = pd.Series(color_ids.values, index=yr_dates)
    day_nums, day_vals = split_months(df_dates, year)
    create_month_calendars(day_nums, day_vals, filename_base)
...
def create_comb_month_calendars(s2_day_nums, l8_9_day_nums, s2_day_vals, l8_9_day_vals, color_dict, filename_base):
    for i in range(0, 12):
        # set up grid
        ...
        # fig.tight_layout()
        # Save to file
        outname = filename_base.replace('png', f'{i:02}.png')
        plt.savefig(outname, dpi=120)
        plt.close('all')

def month_comb_calendar_graph(s2_df_all, l8_9_df_all, yr_dates, filename_base, color_dict):
    date_gen = pd.DataFrame({
        'date': yr_dates
        , 'colorid':0
    })
    ...
    create_comb_month_calendars(s2_day_nums, l8_9_day_nums, s2_day_vals, l8_9_day_vals, color_dict, filename_base)

# BODY
# def main():
if 1:
    print('Loading files and defining paths and acquisition schedules...')
    ...
    print('Generating output by satellite...')
    for sat in ['s2', 'l8_9']:
        print('Satellite:', sat)
        ...
        for year in years:
            ...
            # if wb_ann_calendars:
            ids_all = pd.concat([ids, over_ids]).drop_duplicates()
            # loop over ids
            print('Generating output by id...')
            for id, acq_id in ids.itertuples(index=False, name=None):
                ...
                acqs = acq_id.split('_')
                df_all = ann_calendar_text(acqs, full_paths, part_paths, yr_starts, pass_int)
                 # make csv of dates
                filename = os.path.join(str(year), 'text_calendars', f'{basename}.csv')
                if (not os.path.exists(filename)):
                    df_all.drop(['colorid'], axis=1).to_csv(filename, index=False)
                    text_cal_num+=1
                    if text_cal_num % 100 == 0:
                        print(f'{text_cal_num} text calendars made')
                # make annual calendars
                filename = os.path.join(str(year), 'annual_calendars', f'{basename}.pdf')
                if (not os.path.exists(filename)):
                    ann_calendar_graph(df_all, yr_dates, filename, color_dict, full_paths, part_paths)
                    plt.close('all')
                    ann_cal_num+=1
                    if ann_cal_num % 100 == 0:
                        print(f'{ann_cal_num} annual calendars made')
                # make monthly calendars
                filename = os.path.join(str(year), 'monthly_calendars', f'{basename}12.png')
                if (not os.path.exists(filename)) and (len(part_paths)==0):
                    filename = os.path.join(str(year), 'monthly_calendars', f'{basename}.png')
                    month_calendar_graph(df_all, yr_dates, filename, color_dict)
                    plt.close('all')            
                    mon_cal_num+=1
                    if mon_cal_num % 1200 == 0:
                        print(f'{mon_cal_num} monthly calendars made')
    print('Generating figures of combined satellite calendars...')
    for s2_id, l8_9_id in s2_l8_9_ids:
        for year in years:
            print('Year:', year)
            basename = f's2_{s2_id}_l8_9_{l8_9_id}_{year}'
            filename = os.path.join(str(year), 'combined_monthly_calendars', f'{basename}12.png')
            if not os.path.exists(filename):
                filename = os.path.join(str(year), 'combined_monthly_calendars', f'{basename}.png')
                s2_text_name = os.path.join(str(year), 'text_calendars', f's2_{s2_id}_{year}.csv')
                l8_9_text_name = os.path.join(str(year), 'text_calendars', f'l8_9_{l8_9_id}_{year}.csv')
                s2_df_all = (
                    pd
                ...
                yr_dates = pd.date_range(f'{year}-01-01', end=f'{year}-12-31', freq='D')
                month_comb_calendar_graph(s2_df_all, l8_9_df_all, yr_dates, filename, color_dict)
                plt.close('all')
                comb_cal_num+=1
                if comb_cal_num % 1200 == 0:
                    print(f'{comb_cal_num} combined calendars made')
# if __name__ == "main":
#     main(False
